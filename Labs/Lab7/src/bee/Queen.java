        ...
        while(this.beeHive.isActive()){
            if(this.beeHive.hasResources() && this.beeHive.getQueensChamber().hasDrone()){
                ...
                for(int i = 0; i < newBees; i++){
                    ...
                    if(beeType > 4){
                        Bee drone = Bee.createBee(Role.DRONE, Worker.Resource.NONE, this.beeHive);
                        this.beeHive.addBee(drone);
                    }else{
                        if(beeType > 2){
                            Bee nectar = Bee.createBee(Role.WORKER, Worker.Resource.NECTAR, this.beeHive);
                            this.beeHive.addBee(nectar);
                        }else{
                            Bee pollen = Bee.createBee(Role.WORKER, Worker.Resource.POLLEN, this.beeHive);
                            this.beeHive.addBee(pollen);
                        }
                    }
                }
